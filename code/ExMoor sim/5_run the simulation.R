...
foreach(id = 1:n_IDS, .options.snow = opts) %dopar% {
  ...
  pen <- read.table("data/fake Exmoor site/exmoor_release_pen_coordinates.csv",
                    sep = ",", header = TRUE) %>%
    dplyr::select(X, Y)
  ...
  short_list <- T
  hab <- rast("outputs/script_4/Exmoor site/exmoor cropped habitat raster.tif")
  pen <- rast("outputs/script_4/Exmoor site/exmoor cropped pen distance raster.tif")
  feed <- rast("outputs/script_4/Exmoor site/exmoor cropped feeder distance raster.tif")
  wood <- rast("outputs/script_4/Exmoor site/exmoor cropped wood distance raster.tif")
  
  ## bind all covariate rasters together ####
  covs <- c(feed, hab, wood, pen)
  
  wood_rast <- rast("outputs/script_4/Exmoor site/exmoor cropped wood raster.tif")
  
  ## load in the hedges and egdes rasters ####
  hedges_edges <- rast("outputs/script_4/Exmoor site/exmoor cropped hedges_edges raster.tif")
  hedges_edges_dist <- rast("outputs/script_4/Exmoor site/exmoor cropped hedges_edges distance raster.tif")
  ...
  ## save the simulation ####
  saveRDS(sim_df, paste0("outputs/script_5/Exmoor site/", id, "_sim_output.rds"))
  rm(sim_df)
}; stopCluster(cl)
