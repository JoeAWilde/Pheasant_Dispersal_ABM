...
for(s in site) {
  if(s == site[1]) pb <- progress_bar$new(total = length(site),
                                            ...
                                            clear = F); pb$tick(0)
  root <- paste0("outputs/script_6/PA sites/", manag, "/", s, "_site/")
  ...
  for(d in dists){
    ...
    all_df <- lapply(sim_files, readRDS) %>%
      do.call(rbind, .)
    saveRDS(all_df, paste0("outputs/script_7/PA sites/", manag, "/", ss, "simulation_data.rds"))
  }
  pb$tick()
}
sites <- c()
...
for(ss in sites) {
  ...
  all_sites_df <- readRDS(paste0("outputs/script_7/PA sites/", manag, "/" , ss, "simulation_data.rds"))
  ...
  foreach(i = 1:nrow(grouped_sums), .options.snow = opts) %dopar% {
    ...
    saveRDS(c(i, mean_fixes, birdhours_in_PA), paste0("outputs/script_7/PA sites/", manag, "/summarised data loop output/",
  ...
  saveRDS(summarised,
          paste0("outputs/script_7/PA sites/", manag, "/summarised data/", ss, " summarised data.rds"))
}
