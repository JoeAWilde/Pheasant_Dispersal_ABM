...
for(ss in sites) {
  ...
  all_sites_df <- readRDS(paste0("outputs/script_7/PA sites/", ss, "simulation_data.rds"))
  ...
  foreach(i = 1:nrow(grouped_sums), .options.snow = opts) %dopar% {
    ...
    saveRDS(c(i, mean_fixes, birdhours_in_PA), paste0("outputs/script_7/PA sites/summarised data loop output/",
  ...
  stopCluster(cl)
  mf_bh_root <- "outputs/script_7/PA sites/summarised data loop output/"
  ...
  saveRDS(summarised,
          paste0("outputs/script_7/PA sites/summarised data/", ss, " summarised data.rds"))
}
