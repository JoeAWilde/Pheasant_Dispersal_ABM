...
for(ss in sites) {
  ...
  sim_files <- paste0(root, list.files(root)) %>%
    .[grepl(ss, .) & grepl("_higher_feeder_attraction", .)]
  ...
  all_df <- lapply(sim_files, readRDS) %>%
    do.call(rbind, .)
  saveRDS(all_df, paste0("outputs/PA sites/script_6/", ss, "simulation_data_higher_feeder_attraction.rds"))
  pb$tick()
}
...
for(ss in sites) {
  ...
  all_sites_df <- readRDS(paste0("outputs/PA sites/script_6/", ss, "simulation_data_higher_feeder_attraction.rds"))
  ...
  saveRDS(summarised,
          paste0("outputs/PA sites/script_6/summarised data/", ss, " summarised data_higher_feeder_attraction.rds"))
}
