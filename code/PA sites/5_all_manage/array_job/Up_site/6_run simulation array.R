...
# Read the SLURM task ID (default to 1 if not provided)
args <- commandArgs(trailingOnly = TRUE)
task_id <- ifelse(length(args) > 0, as.integer(args[1]), 1)
...
sl_pars <- readRDS("outputs/script_2/sl_pars.rds")
sl_spec_pars <- readRDS("outputs/script_2/sl_spec_pars.rds")
...
ta_pars <- readRDS("outputs/script_2/ta_pars.rds")
...
ssf_betas <- readRDS("outputs/script_3/ssf_betas.rds")
...
hedges <- rast(paste0("outputs/script_5/PA sites/", ss, " cropped managed hedgerow distance raster.tif"))
...
sim_df <- id_sim(task_id, sl_pars, ta_pars, ssf_betas, cov_names, pen_pts, dogin_dates, dogin_times, 
...
## save the simulation ####
saveRDS(sim_df, paste0("outputs/script_6/PA sites/5_all_manage/Up_site/", task_id, "_sim_output_site_", ss, ".rds"))
