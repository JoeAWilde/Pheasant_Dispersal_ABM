...
pp <- ggplot() + 
  geom_density(data = sim_df, aes(x = dist, fill = "Simulated"),colour = "black", alpha = 0.5) + 
  geom_density(data = real_df, aes(x = dist, fill = "Observed"), colour = "black", alpha = 0.5) + 
  scale_x_continuous(name = "Step lengths (m)", limits = c(0,3000)) + 
...
p1
ggsave(p1, filename = "outputs/script_8/ATLAS data/distance_props.png", units = "px", height = 4320, width = 7980)
...
p2 <- ggplot(data = count_small, aes(x = dist_band_str,
  ...
  facet_wrap(vars(month), axes = "all_x")
p2
ggsave(p2, filename = "outputs/script_8/ATLAS data/distance_counts.png", units = "px", height = 4320, width = 7980)
