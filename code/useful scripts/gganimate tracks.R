...
p1 <- ggplot() +
  ...
  geom_path(data = df, aes(x = X_coord, y = Y_coord, colour = tod, group = ID), linewidth = 1, alpha = 0.7) + 
  geom_point(data = df, aes(x = X_coord, y = Y_coord, colour = tod, group = ID), size = 2) + 
  ...
  scale_y_continuous(name = "Distance from release pen (m)", breaks = NULL, limits = c(min(df$Y_coord)-100, max(df$Y_coord)+100)) +
  scale_x_continuous(name = "Distance from release pen (m)", breaks = NULL, limits = c(min(df$X_coord)-500, max(df$X_coord)+500)) +
